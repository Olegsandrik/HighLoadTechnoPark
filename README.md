...
### Механизмы резервирования БД

* Описаны подробнее ранее. Используется шардирование, реплецирование и backup'ы.

### Механизмы резервирования виртуальных компонент

* K8s использует механизмы встроенные механизмы для обеспечения отказоустойчивости. Он автоматически восстанавливает работу неработающих подов или кластера Kubernetes, запуская новый экземпляр. 

* Nginx разворачивается в виде нескольких экземпляров. Таким образом в случае выхода из строя одного из, остальные распределят нагрузку между собой до того, как "упавший" Nginx не поднимут.

* etcd СУБД хранит всю конфигурацию и состояние кластера Kubernetes и Nginx. При перезапуске всего ДЦ Kubernetes использует etcd для восстановления состояния кластера. После перезапуска контроллеры и планировщики Kubernetes считывают данные из etcd и восстанавливают необходимые ресурсы. При перезапуске всего Nginx может автоматически загружать свою конфигурацию из etcd, обеспечивая быстрое восстановление работы. Также СУБД упрощает централизованную настройку конфигураций.

### Механизмы резервирования физических компонент

* Сервера - реализованы группы по логике, а также используется кластеризация серверов в k8s. Чтобы в случае выхода одно из машин из строя, сервис продолжал корректную работу.

* Диски - используем RAID 5. Данные и информация о четности распределяются по всем дискам. Для восстановления данных при сбое используется информация о четности. Дает хороший баланс между производительностью, емкостью и надежностью. Может потерять один диск без потери данных.

### Мониторинг метрик

* С помощью использования Kafka реализуется сбор метрик в микросервис, привязанный к Prometheus. Такая реализация позволяет производить полноценный мониторинг всего физического состояния системы. В случае каких-либо проблем система автоматически будет оповещать о таковых, что позволит оперативно устранять сбои системы. Хранится будут гистограммы времени отклика, CPU, наличие свободной памяти и скорость чтения/записи дисков.

### Graceful degradation

* Для реализвации данного метода API-GATEWAY в случае отказа какого-либо сервиса он возвращает на фронт сообщение о "временной недоступности". Например сервис комментариев "упал", тогда frontend отобразит не комментарии, а сообщение о том, что комментарии сейчас недоступны. Таким образом система будет более отказоустойчива.

| **Компонент** | **Механизмы резервирования**|
|-----------------------------------|------------------------------------|
| **Базы данных**                   | Используются шардирование, репликация и резервные копии. Подробно описано ранее |
...
| **Graceful degradation**           | Реализуется через API-GATEWAY: в случае отказа сервиса возвращает сообщение о "временной недоступности". Например, если сервис комментариев "упал", фронтэнд отображает сообщение о недоступности комментариев, повышая отказоустойчивость системы.   |
...
![image](https://github.com/user-attachments/assets/f36c82ed-2221-4c6d-97a3-db137c75c655)

### Какие данные передают друг другу сервисы
