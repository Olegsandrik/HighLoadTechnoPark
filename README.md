...
* Для хранения таблиц Films, Film_Genres, Genres, Film_actors, Actors, Photos будет использоваться PostgreSQL+Citus, так как объем данных позволяет пользоваться SQL вариантом и нужно распределить систему на несколько ДЦ.
...
### Физическая схема СУБД до денормализации

![image](https://github.com/user-attachments/assets/e8117858-68b5-4e6a-8a21-39a3c0c17b51)

### Физическая схема СУБД после денормализации

![image](https://github.com/user-attachments/assets/9bfef507-edc1-47c3-9f60-6f45154cb7b0)

В поле genres films записано
```
[
  "genr_1",
  "genr_2",
...
В поле actors_with_roles films записано
```
[
  {"name_sunname": "Stas Vasilyiev", "role": "murderer", "photo_url": "http://.../photo1.jpg", "id": "123"},
  {"name_sunname": "Artem Lebedev", "role": "design", "photo_url": "http://.../photo2.jpg", "id": "12345"},
  ....
]

```

В поле photos actors записано
```
[
  "url_1",
  "url_2",
...
Такое решеиние обосновано тем, что у нас в приоритете именно чтение, а не запись. Так как любой человек заходя на страницу фильма получает данные об актерах, их ролях и фотографияю, а также заходя на страницу актера получает все его фотографии. Поэтому не потребуется выполнять операцию join несколько раз. Что снизит нагрузку на СУБД.
...
**Нужно доделать:
Посчитать таблицу Расчет размера данных и нагрузки на чтение/запись
