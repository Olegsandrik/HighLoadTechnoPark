...
| Genres | ~150 байт * 100 =  **0.000014 GB** | удаляюся все Film_Genres | | |
...
| Film_Geners | ~12 байт * 160 тысяч единиц контента * 3 = **0.00534 GB**| - | | 2.5 млн * 1 /84600 |
| Session |  ~324 байта * 365 * 2 500 000 = **275.6 ГБ** | - | | 29 |

## Физическая схема БД
### Выбор СУБД
* Для поиска фильмов по названию и описанию будет использоваться **Elasticsearch**.
* Для хранения таблиц Films, Genres, Actors, Films_Actors будет использоваться **PostgreSQL**, так как нужно распределить систему на несколько ДЦ.
...
* Для хранения таблиц Users, Reviews использоваться будет Cassandra, так как более 25 млн пользователей и обзоров > 180 млн.
* Для Avatars, Photos и видеоконтента использоваться будет **Minio**.
...
Redis используется для управления временными данными сессий. Каждая сессия хранится как набор ключ-значение, где ключом является session_id, а значениями — данные сессии, такие как user_id и время входа.
...
![image](https://github.com/user-attachments/assets/544b09c5-24e2-48cc-8efc-ef0702018b05)
...
```
Остальные типы данных атрибутов всех сущностей являются примитивными типами данных
...
**Шардирование**: Elasticsearch, Tarantool, Cassandra, ClickHouse, Kafka изначально поддерживает шардирование. Minio поддерживает распределение данных по нескольким серверам для повышения отказоустойчивости, Redis Cluster позволяет распределять данные по нескольким узлам. Prometheus, PostgreSQL и ectd не поддерживают шардирование из коробки, однако с использованием federation или Cortex/Thanos Prometheus годится для распределенного сбора метрик, а у ectd можно создать несколько кластеров для разных задач.
Везде добавляются новые узлы, кроме kafka и Prometheus, где добавляются новые брокеры и используются несколько инстансов для горизонтального масштабирования.
...
**etcd**: Python - etcd3 (для более обширного комьюнити)
...
**PostgreSQL**: Поскольку в большинстве ДЦ установлены реплики, то главное создать backup master'a. Мастер находится в Мск и каждую неделю создается backup, хранящийся 3 месяца.
**Elasticsearch**: Для резервного копирования будем использовать snapshot и restore Elasticsearch, которые будем сохранять в локальном ДЦ. 
**Tarantool**: Раз в неделю производим дамп данных и WAL-файл и записываем в локальный ДЦ. 
**Cassandra**: Используем nodetool snapshot для создания снимков данных в локальный ДЦ.
**Redis**: Используем механизм создания снапшотов (RDB) и журналирования (AOF) для резервного копирования данных Redis. Еженедельно копируем файлы RDB/AOF в локальный ДЦ.
**ClickHouse**: Используем clickhouse-backup для создания резервных копий данных и метаданных в локальный ДЦ.
**Prometheus**: Используем встроенные механизмы резервного копирования данных Prometheus, такие как копирование файлов данных, перенося их в локальный ДЦ.
**etcd**: Используем встроенные команды для создания резервных копий, такие как etcdctl snapshot save, сохраняя эти копии в локальный ДЦ.
**Kafka**: используем kafka-backup для создания резервных копий данных из Kafka в локальный ДЦ.
**Minio**: Используем встроенные инструменты Minio для создания резервных копий, такие как mc mirror для синхронизации данных с другим хранилищем. Репликация данных между несколькими кластерами. Это обеспечит дополнительную защиту данных в случае сбоя одного из кластеров, повышая отказоустойчивость. В последствии часть данных можно восстановить. Так как в minio в основном храняться одни и те же видеоматериалы, при полном копировании данных с соседнего кластера ничего не измениться. 
Также в Московском дц будут собраны все бекапы всех СУБД, кроме Minio, которая по размерам кратно превосходит остальные.
