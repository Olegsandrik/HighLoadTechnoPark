...
* K8s
Kubernetes использует механизмы встроенные механизмы для обеспечения отказоустойчивости. Он автоматически восстанавливает работу неработающих подов или кластера Kubernetes, запуская новый экземпляр. 
...
* etcd
Эта СУБД хранит всю конфигурацию и состояние кластера Kubernetes и Nginx. При перезапуске серверов или всего ДЦ Kubernetes использует etcd для восстановления состояния кластера. После перезапуска контроллеры и планировщики Kubernetes считывают данные из etcd и восстанавливают необходимые ресурсы. При перезапуске Nginx может автоматически загружать свою конфигурацию из etcd, обеспечивая быстрое восстановление работы. Также СУБД упрощает централизованную настройку конфигураций.
